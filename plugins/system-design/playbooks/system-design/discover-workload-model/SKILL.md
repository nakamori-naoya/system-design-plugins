...
`write-doc`は契約ID `write-doc/write-doc`の公開playbookとして呼び、`material`の絶対path、媒体`markdown`、日本語の本文題名と保存用ASCII名`workload-model.md`を区別して渡す。呼出側が公開入口E1で作ったwrite-doc用設定だけを呼出側が保持し、最後の利用後に成功・失敗のどちらでも後片付けする。write-docの内部skill、references、非公開scriptは参照しない。
