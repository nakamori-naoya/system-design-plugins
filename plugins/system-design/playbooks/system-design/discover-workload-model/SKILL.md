...
description: 調査した利用規模、分布、ピーク、増加、偏りを採用仮定へ変え、要件とインフラ構成の判断へ接続した利用・負荷モデルを保存する独立プレイブック。負荷を設計入力にしたいときに使う。
---

# discover-workload-model

同梱の`playbook.yml`を公開入口とし、内部プラグイン`discover-workload-model`の同名スキルへ明示的にルーティングする。SLO、データモデル、クラウド構成は決めない。

## 実行

1. `scripts/prepare.sh`で、このプレイブックと対応スキルが同じ配布パッケージ内にあることを検査する。
2. `playbook.yml`の`steps`に従い、grill、ground、`skill: discover-workload-model`、material、verify、write-docを順に実行する。
3. 成否にかかわらずcleanupを実行し、保存済み正本と参照資料を保持する。`scripts/resolve.sh`は公開入口と内部スキルの対応を機械可読なJSONとして確認する。

入力、出力、停止条件、非責務の詳細は、同梱内部スキル`skills/discover-workload-model/SKILL.md`を正本とする。
...
`write-doc/write-doc` 契約 v2 の公開入口へ、`document` 工程の入力を直接渡す。`material` は生成済み素材の絶対パスを `[{kind: file, path: <materialの絶対パス>}]` に変換する。`document_type: workload-model` を維持し、新規作成は `output_directory` と `name: workload-model.md`、更新は `update_target` だけを加える。日本語の本文題名は素材に保持する。更新時は `name` と `output_directory` を渡さない。入力・出力用の中間ファイルや相手用の実行設定は作らず、返された `status` と `path` または `reason` を直接受け取る。

`status: completed` は保存成功だけを示す。返却された絶対パスの文書を読み、素材のID、根拠、数値、未決、`handoff`、後続成果物への追跡が保持されていることを確認してから `workload_document_path` へ割り当てる。検証結果の `ready`／`unresolved` を `completed` に置き換えない。保存成功と内容保持を確認できた場合だけ `contract.outcome` を返す。
