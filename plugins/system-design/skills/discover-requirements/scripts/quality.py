...
読む目印: write-doc の quality-requirements 型の template にある「検査が読む目印」。見出しの文言は読まない。後続資料が
  IDで参照する品質要求と未決は、見出し行が「ID | 守る性質 | 観測・検証 | 根拠と状態」の追跡の表に置く（どの見出しの下でもよい）。
  上流（要件・利用負荷モデル）が定義するID。
...
正例: tests/fixtures/quality-requirements.md（write-doc の見本と同じ本文。--upstream に要件と利用負荷のfixture）。
...
def main() -> int:
    ...
    parser.add_argument("--upstream", action="append", default=[], help="上流資料（要件・利用負荷モデル）の絶対path。複数可")
