...
各入力へ`SRC-` IDを付け、`kind`、絶対pathまたは再現可能な所在情報、版またはハッシュ、観測時点を記録する。各品質根拠主張へ`CLM-` IDを付け、判断基準で分類し、根拠成果物、根拠内ID、観測時点、品質区分へ結ぶ。`fact`か`agreed_decision`か判定できない主張は`hypothesis`に分類し、分類が閾値の合意状態を変えるなら推奨付きの未決の問いへ結ぶ。
...
### 7. 問いを厳選して確認し、残りは推奨を仮置きして一覧を確認する

手順1〜6で残った未決のうち、上流成果物と根拠を読めば分かることは問わずに読んで決める。答えで観測点、閾値、母集団、時間窓、矛盾の解消、アーキテクチャへの引き渡しが変わる問いだけを選び、1回の実行で問うのは最大6問にする。選んだ問いは理由付きの推奨を添えて一問ずつ確認し、回答を受理した問いを`resolved`、理由付きで取り下げられた問いを`withdrawn`にする。

7問目以降と、問わないと決めた論点は`state: open`のまま残し、`reason`にその時点の根拠から立てた推奨、その根拠、採らなかった解釈を書く。推奨の閾値は正本へ`hypothesis`として反映し、`agreed`へ昇格させない。測定条件が揃わないQRは推奨を添えても`unresolved`のままにし、矛盾は一方の数値を上書きせず`open`のままにする。
...
- `quality.py`、`terminology.py`、`write-doc`が失敗した。理由を変えずに返す。

止まるときは確認済み根拠主張と測定条件を残し、止めた判断、必要な入力または確認、回答者または検証者、影響するQR/QWL/QCON ID、再開条件を返す。

手順7の一覧確認の回答待ちは継続待ちであり、完了でも失敗でもない。待つ間は正本を保存しない。

判断の揺れや資料の不足では、その時点の根拠から最も筋の良い仮説を立てて進む。

- 観測点、指標、母集団に複数の解釈があり得る: 最も筋の良い解釈で`QR-`を起こし、閾値は`hypothesis`にし、採らなかった解釈を推奨付きの未決の問いへ残す。
- 根拠主張の状態、観測時点、決定者が無い: `hypothesis`に分類し、誰がいつ合意または反証するかを未決の問いにする。
- 利用・負荷モデルが無い、または版・指標状態が分からない: 負荷依存のQRを`assumption`または`blocked_by`の`QWL-`で結び、`handoff.ready: false`で保存する。
- 測定条件の一部が欠ける: そのQRを`unresolved`（閾値null）にし、欠けた項目と検証者を推奨付きの未決の問いへ結ぶ。

仮説は仮説と分かる形で正本に書き、業界標準や現在実績を根拠無しに`agreed`へ変えない。
...
- 報告: 正本の絶対path、成果物の状態と版、合意済み・仮説・未決件数、未解消矛盾、問うた問いと回答、推奨を仮置きした未決の問い、作業を止める問い、アーキテクチャへ引き渡せるか、実行した検証、未検証範囲。負荷試験、障害試験、セキュリティ試験、費用計測は実行していないので、未実施の検証を合格として報告しない。
