...
- 基準資料の記法（見出し・表の列・ID）はwrite-docの各文書型templateが所有する。このrepositoryの検査scriptはその記法を読み、記法とscriptが食い違えばscript側を直す。package共有code（`plugins/system-design/scripts/`）は構文解析と用語定義の検査だけを持ち、各skill固有の意味判断を共通層へ移さない。
